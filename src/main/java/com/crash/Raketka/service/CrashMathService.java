...
public class CrashMathService {
    ...
    public int computeLevelsPassed(double currentMultiplier, GameConfig config, Theme theme) {
        GameConfig.ThemeConfig themeConfig = requireThemeConfig(theme, config);
        List<Double> boosterMultipliers = themeConfig.getBoosterMultipliers();

        if (boosterMultipliers == null || boosterMultipliers.isEmpty()) {
            return 0;
        }

        int levelsTotal = Math.min(themeConfig.getLevelsTotal(), boosterMultipliers.size());
        int passed = 0;
        for (int i = 0; i < levelsTotal; i++) {
            if (currentMultiplier >= boosterMultipliers.get(i)) {
                passed++;
            }
        }
        return passed;
    ...
    public int computePointsEarned(int levelsPassed, Double cashoutMultiplier, boolean boosterTriggered,
                                   GameConfig config) {
