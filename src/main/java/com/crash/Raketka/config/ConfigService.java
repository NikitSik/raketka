    ...
    private void validate(GameConfig config) {
        ...
        for (Map.Entry<String, GameConfig.ThemeConfig> entry : themes.entrySet()) {
            ...
            if (boosterMultipliers == null || boosterMultipliers.isEmpty()) {
                throw new IllegalArgumentException(
                        "theme " + themeName + " has empty boosterMultipliers");
            }
            if (lineProbabilities.size() != boosterMultipliers.size()) {
                throw new IllegalArgumentException(
                        "theme " + themeName + " lineProbabilities.size() (" + lineProbabilities.size()
                                + ") must equal boosterMultipliers.size() (" + boosterMultipliers.size() + ")");
            }
        }
